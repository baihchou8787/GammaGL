...
def fit_tokenizer(args, train_graphs: Sequence[SyntheticGraph]):
    ...
    from gammagl.transforms.graph_tokenizer import GraphTokenizerSpecialTokens
    ...
    if getattr(args, "protocol", None) == "paper" and getattr(args, "model", None) == "gte":
        # The pinned GTE checkpoint fixes padding_idx=1.  Keep the eight
        # special IDs contiguous and distinct while preserving BERT's PAD=0.
        special_tokens = GraphTokenizerSpecialTokens(
            pad_token_id=1,
            unk_token_id=0,
        )
...
def prepare_paper_datasets(data_root, datasets: Sequence[str]) -> List[Dict[str, Any]]:
    """Single-process installation of immutable paper data before training."""
    ensure_repo_on_path()
    ...
    for dataset in datasets:
        spec = resolve_dataset_spec(dataset)
        raw_dir = Path(data_root) / spec.canonical_name / "raw"
        ...
        prepared.append(preflight_dataset_status(data_root, dataset))
    return prepared
...
def preflight_model_status(model: str) -> Dict[str, Any]:
    ...
    if model_name == "gte":
        status.update({
            ...
            "pretrained_model_id": "Alibaba-NLP/gte-multilingual-base",
            "pretrained_revision": "9bbca17d9273fd0d03d5725c7a4b0f6b45142062",
        })
    return status
...
def run_paper_experiments(args):
    ...
    for run in paper_summary["runs"]:
        runs.append({
            ...
            "loss_space": run["loss_space"],
            "metric_space": run["metric_space"],
            "target_normalization": run["target_normalization"],
        })
    ...
    summary = {
        ...
        "manifest": {
            **build_runtime_manifest(args),
            "loss_space": paper_summary["loss_space"],
            "metric_space": paper_summary["metric_space"],
            "target_normalization": paper_summary["target_normalization"],
        },
    }
    summary["outputs"] = write_experiment_outputs(summary, args)
    return summary
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gte-cache-dir", default=None,
        help="Optional cache for the pinned official GTE checkpoint.",
    )
    ...
    parser.add_argument(
        ...
        help="Single-process download, verify, materialize, and split-check paper datasets.",
    )
...
def main(argv=None):
    ...
    if args.prepare_data:
        datasets = parse_csv_values(args.datasets) or [args.dataset]
        report = {"status": "ok", "datasets": prepare_paper_datasets(args.data_root, datasets)}
        print(json.dumps(report, indent=2))
        return report
    if args.smoke:
